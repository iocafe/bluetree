...
*/
#include "egui.h"
#include "styleserializer/imguistyleserializer.h"
...
****************************************************************************************************
  ...
  The eGui::make_autolabel generates unique label for ImGui windows.

  @param   imgui_name Pointer to variable where to store unique name.

****************************************************************************************************
*/
void eGui::make_autolabel(
    eComponent *c,
    eVariable *name,
    eVariable *imgui_name)
{
    ...
    if (m_autolabel == OS_NULL) {
        m_autolabel = new eContainer(this, EOID_ITEM, EOBJ_IS_ATTACHMENT);
        m_autolabel->ns_create();
    }

    fullname.sets(c->classname());
    fullname.appends("-");
    if (name) fullname.appendv(name);

    m = eMatrix::cast(m_autolabel->byname(fullname.gets()));
    if (m == OS_NULL) {
        m = new eMatrix(m_autolabel);
        m->addname(fullname.gets());
        m->allocate(OS_BOOLEAN, 32, 1);
    }

    for (i = 0; i < 100; i++) {
        if (!m->geti(i, 0)) {
            m->setl(i, 0, OS_TRUE);
            osal_int_to_str(nbuf, sizeof(nbuf), i);
            imgui_name->sets(nbuf);
            imgui_name->appendv(&fullname);
            return;
        }
    }

    imgui_name->setv(&fullname);
    osal_debug_error("too many windows");
}
...
****************************************************************************************************
  ...
  @brief Release ImGui autolabel.

  The eGui::release_autolabel releases unique label generated by make_autolabel.

  @param   imgui_name Unique imgui name.

****************************************************************************************************
*/
void eGui::release_autolabel(
    const os_char *imgui_name)
{
    ...
    if (m_autolabel == OS_NULL) return;

    p = imgui_name;
    i = (os_int)osal_str_to_int(p, &count);
    if (count <= 0) return;
    p += count;

    m = eMatrix::cast(m_autolabel->byname(p));
    if (m == OS_NULL) return;
